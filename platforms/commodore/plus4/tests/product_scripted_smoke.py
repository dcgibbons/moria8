...
C128_TESTS_DIR = REPO_ROOT / "platforms" / "commodore" / "c128" / "tests"
...
def main() -> int:
    ...
    args.swap3_addr = None
    if args.swap3_symbol:
        args.swap3_addr = symbols.get(args.swap3_symbol)
        if not args.swap3_addr:
            print(f"FAIL: missing symbol {args.swap3_symbol} in {args.main_vs}")
            return 2
        if not args.swap3_attach8_d64:
            print("FAIL: --swap3-symbol requires --swap3-attach8-d64")
            return 2
        args.swap3_addr = normalize_addr(args.swap3_addr)
    args.swap4_addr = None
    if args.swap4_symbol:
        args.swap4_addr = symbols.get(args.swap4_symbol)
        if not args.swap4_addr:
            print(f"FAIL: missing symbol {args.swap4_symbol} in {args.main_vs}")
            return 2
        if not args.swap4_attach8_d64:
            print("FAIL: --swap4-symbol requires --swap4-attach8-d64")
            return 2
        args.swap4_addr = normalize_addr(args.swap4_addr)

...
BATCH_TESTS: dict[str, TestCase] = {
    ...
    "look_trampoline128": TestCase("look_trampoline128", SCRIPT_DIR / "tests" / "test_look_trampoline128.s", 5.0),
...
def ensure_kickass(kickass: Path, custom_path: bool) -> None:
    command = [
        ...
        "-s",
        "-C",
        str(REPO_ROOT / "platforms" / "commodore"),
        "ensure-kickass",
    ]
    if custom_path:
...
def assemble_if_stale(
    ...
    prg_path = artifact_dir / source.with_suffix(".prg").name
    vs_path = artifact_dir / source.with_suffix(".vs").name
    ...
    command = [
        ...
        str(REPO_ROOT / "core"),
        "-libdir",
        str(REPO_ROOT / "platforms/commodore/common"),
        "-libdir",
        str(REPO_ROOT / "platforms/commodore"),
        "-libdir",
        str(REPO_ROOT / "platforms/commodore/c64"),
        "-define",
        "C128",
...
def build_batch_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--snapshot-path", default=str(REPO_ROOT / "build" / "test" / "c128" / "ready.vsf"))
    parser.add_argument(
        "--artifact-dir",
        default=str(REPO_ROOT / "build" / "test" / "c128" / "unit"),
        help="Directory for assembled unit .prg/.vs/.sym artifacts",
    )
    parser.add_argument(
        "--tests",
...
def cleanup_source_symbols() -> None:
    for path in (SCRIPT_DIR / "tests").glob("*.sym"):
        path.unlink()

...
tsc_spell_exec_calls: .byte 0
tsc_huff_calls: .byte 0
tsc_last_huff_id: .byte 0
tsc_last_spell_idx: .byte $ff
tsc_kill_calls: .byte 0
...
test_tramp_stinking_cloud_execute:
    inc tsc_spell_exec_calls
    lda pm_spell_idx
    sta tsc_last_spell_idx
    lda #9
    jsr eff_ball
    rts
...
test_reset_stinking_cloud_state:
    jsr player_init
    lda #0
    sta tsc_spell_exec_calls
    sta tsc_huff_calls
    sta tsc_last_huff_id
    sta tsc_kill_calls
    lda #$ff
    sta tsc_last_spell_idx
...
test_start:
    ...
    :PatchJump(test_spell_execute_selected, test_tramp_stinking_cloud_execute)
    :PatchJump(pm_select_book, test_pm_select_book)
    :PatchJump(pm_prompt_visible_spell_choice, test_pm_prompt_visible_spell_choice)
    :PatchJump(pm_validate_selected_spell, test_pm_validate_selected_spell)

    // Test 1: successful cast reaches spell slot 6, remains message-light,
    // kills a monster in the target area, spends 3 mana, and marks worked.
    :PatchJump(calc_spell_failure, test_calc_spell_failure_success)
    jsr test_reset_stinking_cloud_state
    ...
    bcc !t1_fail+
    lda tsc_spell_exec_calls
    cmp #1
    bne !t1_fail+
    lda tsc_last_spell_idx
    cmp #6
    bne !t1_fail+
    lda tsc_huff_calls
    bne !t1_fail+
...
!t1_fail:
    ...
    // Test 2: empty target area stays message-light/no-effect while still
    // consuming mana and marking the spell worked on success.
!t2:
    :PatchJump(calc_spell_failure, test_calc_spell_failure_success)
    jsr test_reset_stinking_cloud_state
    jsr tv_setup_dark_room
...
!t2_fail:
    ...
    // and leaves Stinking Cloud unworked.
!t3:

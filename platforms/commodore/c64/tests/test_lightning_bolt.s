// test_lightning_bolt.s — Focused runtime tests for the Lightning Bolt spell row
...
// Slimmed UI stubs — bolt tests never drive inventory/equipment/character UI.
ui_inv_display:
...
tlb_spell_exec_calls: .byte 0
tlb_huff_calls: .byte 0
tlb_last_huff_id: .byte 0
tlb_last_spell_idx: .byte $ff
tlb_kill_calls: .byte 0
...
test_tramp_lightning_bolt_execute:
    inc tlb_spell_exec_calls
    lda pm_spell_idx
    sta tlb_last_spell_idx
    lda #3
    ldx #8
    ldy #1
    jsr eff_bolt
...
test_reset_lightning_bolt_state:
    jsr player_init
    lda #0
    sta tlb_spell_exec_calls
    sta tlb_huff_calls
    sta tlb_last_huff_id
    sta tlb_kill_calls
    lda #$ff
    sta tlb_last_spell_idx
...
test_start:
    ...
    :PatchJump(test_spell_execute_selected, test_tramp_lightning_bolt_execute)
    ...
    bcc !t1_fail+
    lda tlb_spell_exec_calls
    cmp #1
    bne !t1_fail+
    lda tlb_last_spell_idx
    cmp #8
    bne !t1_fail+
    lda tlb_huff_calls
    bne !t1_fail+

// test_dispel_undead_prayer.s — Focused runtime tests for the Dispel Undead prayer row
...
tdup_spell_exec_calls: .byte 0
tdup_huff_calls: .byte 0
tdup_last_huff_id: .byte 0
tdup_last_spell_idx: .byte $ff
tdup_kill_calls: .byte 0
tdup_rng_value: .byte 0
pmx_work_idx: .byte 0
...
test_tramp_dispel_undead_prayer_execute:
    inc tdup_spell_exec_calls
    lda pm_spell_idx
    sta tdup_last_spell_idx
    lda #CF_UNDEAD
    sta pmx_work_flag
    lda zp_player_lvl
    asl
...
test_reset_dispel_undead_prayer_state:
    jsr player_init
    jsr test_clear_monsters
    lda #0
    sta tdup_spell_exec_calls
    sta tdup_huff_calls
    sta tdup_last_huff_id
    sta tdup_kill_calls
    sta tdup_rng_value
    lda #$ff
    sta tdup_last_spell_idx
...
test_start:
    ...
    :PatchJump(test_spell_execute_selected, test_tramp_dispel_undead_prayer_execute)
    ...
    lda #$03
    sta tc_results + 0
    lda tdup_spell_exec_calls
    cmp #1
    bne !t1_fail+
    lda #$04
    sta tc_results + 0
    lda tdup_last_spell_idx
    cmp #26
    bne !t1_fail+
    lda #$05
    sta tc_results + 0

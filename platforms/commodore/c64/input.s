...
#define HAL_INPUT_FLUSH_RUN_CANCEL_BUFFER
...
// Called by input_get_key / input_wait_release on EVERY keypress, so this must
// stay resident (the init-only tail is overwritten by the title art and map).
// Preserves: X, Y
...
// Preserves: X, Y
input_run_key_held:
#if C64_TEST_SCRIPTED_SPELL || C64_TEST_SCRIPTED_DISK_SETUP_PRODUCT || C64_TEST_SCRIPTED_DISK_SETUP_SINGLE_DRIVE_RETURN_PRODUCT || C64_TEST_SCRIPTED_DISK_SETUP_THEN_LOAD_PRODUCT || C64_TEST_SCRIPTED_SAVE_WRITE_PRODUCT || C64_TEST_SCRIPTED_SLOT2_SAVE_PRODUCT || C64_TEST_SCRIPTED_SAVE_MEDIA_FAIL_PRODUCT || C64_TEST_SCRIPTED_CHANGE_SAVE_DRIVE_PRODUCT || C64_TEST_SCRIPTED_LOAD_RESUME_PRODUCT || C64_TEST_SCRIPTED_SLOT2_LOAD_PRODUCT || C64_TEST_SCRIPTED_SINGLE_DRIVE_LOAD_RETURN_PRODUCT || C64_TEST_SCRIPTED_SINGLE_DRIVE_LOAD_WRONG_MEDIA_PRODUCT || C64_TEST_SCRIPTED_SINGLE_DRIVE_LOAD_CORRUPT_PRODUCT || C64_TEST_SCRIPTED_SINGLE_DRIVE_SAVE_WRONG_MEDIA_PRODUCT || C64_TEST_SCRIPTED_SINGLE_DRIVE_FRESH_SAVE_PRODUCT || C64_TEST_SCRIPTED_LOAD_THEN_SAVE_NEW_EMPTY_PRODUCT || C64_TEST_SCRIPTED_LOAD_MISSING_SAVE_PRODUCT || C64_TEST_SCRIPTED_DUNGEON_ASCENT_PRODUCT || C64_TEST_SCRIPTED_RETIREMENT_PRODUCT || C64_TEST_SCRIPTED_WIZARD_REVEAL_PRODUCT || C64_TEST_SCRIPTED_WIZARD_ITEM_PRODUCT || C64_TEST_SCRIPTED_SCROLL_TELEPORT_PRODUCT || C64_TEST_SCRIPTED_CHEST_OPEN_PRODUCT
    lda #0
    rts
#else
...
#else
#if C64_TEST_SCRIPTED_LOAD_RESUME_PRODUCT || C64_TEST_SCRIPTED_SLOT2_LOAD_PRODUCT
...
c64_test_script_exhausted_wait:
    jmp c64_test_script_exhausted_wait
#else
    ...
    jmp c64_test_wizard_reveal_fail_input_sym
#else
...
c64_test_input_script:
    ...
    .byte $20              // SPACE = acknowledge save-disk prompt after Load
    .byte $00
#else
    ...
    .byte $0d              // RETURN
    .byte $33              // 3 = done
    .byte $32              // 2 = load non-default slot
    ...
    .byte $59              // Y = overwrite non-default slot
    ...
    .byte $00
#else
#if C64_TEST_SCRIPTED_SLOT2_LOAD_PRODUCT
    .byte $4c              // L = load from title
    .byte $32              // 2 = save drive if Disk Setup prompts
    .byte $39              // 9
    .byte $0d              // RETURN
    .byte $33              // 3 = done
    .byte $32              // 2 = load from save slot 2
    .byte $00
#else
...
#if C64_TEST_SCRIPTED_SINGLE_DRIVE_FRESH_SAVE_PRODUCT
    .byte $20              // SPACE = acknowledge insert-save-disk prompt
    .byte $20              // SPACE = acknowledge setup insert-save-disk prompt
    ...
    .byte $4f              // O = open
    .byte $4c              // L = east, toward the fixture chest
    .byte $00
#else
#if C64_TEST_SCRIPTED_SCROLL_TELEPORT_PRODUCT
    .byte $4e              // N = New
    .byte $41              // A = race
    .byte $0d              // RETURN = accept stats
    .byte $42              // B = mage
    ...
    .byte $4c              // L = step onto town stairs
    .byte $3e              // > = descend into dungeon
    .byte $20              // SPACE = dismiss any level-entry message
    .byte $17              // CTRL+W = wizard mode
    .byte $59              // Y = confirm wizard enable
    .byte $20              // SPACE = dismiss any pending message
    .byte $17              // CTRL+W = open wizard menu
    .byte $47              // G = generate item
    .byte $31              // 1
    .byte $32              // 2
    .byte $38              // 8 = Small Wooden Chest
    .byte $0d              // RETURN
    .byte $17              // CTRL+W = open wizard menu
    .byte $47              // G = generate item
    .byte $31              // 1
    ...
    .byte $31              // 1
    .byte $0d              // RETURN
    .byte $20              // SPACE = dismiss done message
    .byte $20              // SPACE = dismiss any monster-sighting message
    .byte $52              // R = read scroll
    .byte $41              // A = first scroll slot
    .byte $00
#else
    ...
    .byte $31              // 1
    .byte $32              // 2
    .byte $38              // 8 = Small Wooden Chest (ID 128)
    .byte $0d              // RETURN
    .byte $17              // CTRL+W = open wizard menu
    .byte $47              // G = generate item
    .byte $39              // 9
...
#else
#if C64_TEST_SCRIPTED_RETIREMENT_PRODUCT
    .byte $20              // SPACE = dismiss royal screen
    .byte $00
#else
    .byte $4e              // N = New
    .byte $41              // A = race

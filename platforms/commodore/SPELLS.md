...
- Shared family coverage added during hardening:
  ...
  - directional control:
    ...
    - `Polymorph Other` now also has row-level coverage on `C64+C128` for shared directional line tracing to a ranged target, silent successful target replacement, preserved occupied-tile invariants, durable scene-redraw request, silent no-target behavior, and cast-fail/success bookkeeping
...
- Current focused runtime suites:
  ...
  - `commodore/c64/tests/test_word_of_destruction.s`
    - `Word of Destruction` spell-id mapping plus upstream area-devastation: 15-radius terrain reroll with metric-boundary probes, silent monster/item/glyph/trap deletion (Balrog deletion is not a win), room darkening, blindness incl. 255 clamp, town guard, and wraparound safety
  ...
  - `commodore/c64/tests/test_polymorph_other.s`
    - `Polymorph Other` spell-id mapping, shared directional line tracing, deterministic ranged target replacement behavior, and durable monster-redraw request
  ...
  - `commodore/c64/tests/test_teleport_other.s`
    - `Teleport Other` spell-id mapping, deterministic target relocation behavior, and durable monster-redraw request
  ...
  - `commodore/c128/tests/test_word_of_destruction128.s`
    - `Word of Destruction` spell-id mapping plus upstream area-devastation against test-local tables: 15-radius terrain reroll, silent monster/item/glyph/trap deletion, room darkening, and blindness
    - `Word of Destruction` redraw updates and cast-fail/success bookkeeping
...
| 12 | Legacy | M2 | Cure Poison | 5/5/35 | 21/9/90 | 11/9/45 | Clear poison | Row-covered on `C64+C128`: focused coverage proves spell-id mapping, poisoned-state clearing with recovery feedback, already-clear silence/no-op behavior, and cast-fail/success mana/worked bookkeeping |
...
| 20 | Added | M3 | Polymorph Other | 11/7/45 | -- | 21/19/60 | Replace target monster with another | Row-covered on `C64+C128`: focused coverage proves the shared directional helper traces through the chosen line to a ranged target, then performs silent successful target replacement at the same coordinates with occupied-tile invariants preserved, requests a durable post-turn scene redraw without faking a room reveal, preserves silent no-target behavior, and maintains cast-fail/success mana/worked bookkeeping |
...
| 27 | Added | M4 | Teleport Other | 23/12/60 | -- | 31/25/70 | Teleport target monster away | Row-covered on `C64+C128`: focused coverage proves deterministic target relocation with occupied-tile invariants preserved, sleep cleared on the moved monster, durable post-turn scene redraw requested without faking a room reveal, silent no-target behavior, and cast-fail/success mana/worked bookkeeping |
...
| 30 | Added | M4 | Word of Destruction | 33/21/80 | -- | 37/30/95 | Destroy surrounding area | Row-covered on `C64+C128`: focused coverage proves upstream 15-radius area-devastation (terrain reroll with metric-boundary probes, silent monster/item/glyph/trap deletion, no winner on deleted Balrog, room darkening, blindness with 255 clamp, town guard), redraw updates, and cast-fail/success mana/worked bookkeeping |
...
| 17 | Added | P3 | Neutralize Poison | 9/6/38 | 15/15/50 | Clear poison | Row-covered on `C64+C128`: focused coverage proves prayer-id mapping, poisoned-state clearing with recovery feedback, already-clear silence/no-op behavior, and cast-fail/success mana/worked bookkeeping |
...
| 25 | Added | P3 | Turn Undead | 15/12/50 | 27/21/50 | Turn undead in sight | Row-covered on `C64+C128`: focused prayer-path coverage proves prayer-id mapping, visible/LOS undead targeting, low-level undead turn with `MX_CONFUSE = player level` and `MX_SLEEP_CUR = 0`, high-level undead can report unaffected, hidden undead remain unchanged, no-visible-undead prints `Nothing seems to happen.`, and cast-fail/success mana/worked bookkeeping |
...
- Documented model-scope deviations, not active spell backlog:
  - make `Resist Heat and Cold` roll independent heat/fire and cold durations if exact `umoria` duration parity matters; broader fire/cold damage consumers remain outside the current modeled engine and this has negligible current end-user impact

// test_cure_critical_wounds_prayer.s — Focused runtime tests for the Cure Critical Wounds prayer row
...
tccwp_spell_exec_calls: .byte 0
tccwp_huff_calls: .byte 0
tccwp_last_huff_id: .byte 0
tccwp_last_spell_idx: .byte $ff
...
test_huff_print_msg:
    stx tccwp_last_huff_id
    inc tccwp_huff_calls
    rts
...
test_tramp_cure_critical_wounds_prayer_execute:
    inc tccwp_spell_exec_calls
    lda pm_spell_idx
    sta tccwp_last_spell_idx
...
test_reset_cure_critical_wounds_prayer_state:
    jsr player_init
    lda #0
    sta tccwp_spell_exec_calls
    sta tccwp_huff_calls
    sta tccwp_last_huff_id
    lda #$ff
    sta tccwp_last_spell_idx
...
test_start:
    ...
    :PatchJump(test_spell_execute_selected, test_tramp_cure_critical_wounds_prayer_execute)
    ...
    jsr player_pray
    bcc !t1_fail+
    lda tccwp_spell_exec_calls
    cmp #1
    bne !t1_fail+
    lda tccwp_last_spell_idx
    cmp #23
    bne !t1_fail+
    lda zp_player_hp_lo
    cmp #26
    ...
    cmp #26
    bne !t1_fail+
    lda tccwp_huff_calls
    cmp #1
    bne !t1_fail+
    lda tccwp_last_huff_id

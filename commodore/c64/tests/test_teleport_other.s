// test_teleport_other.s — Focused runtime tests for the Teleport Other spell row

.pc = $0801 "BASIC Stub"
:BasicUpstart2(test_bootstrap)

.pc = $E000 "Result Buffer"
tto_results: .fill 3, $ff
...
tto_spell_exec_calls: .byte 0
tto_huff_calls: .byte 0
tto_last_huff_id: .byte 0
tto_last_spell_idx: .byte $ff
tto_find_random_floor_calls: .byte 0
tto_work_idx: .byte 0
tto_work_x: .byte 0
tto_work_y: .byte 0
...
test_find_random_floor_teleport_other:
    inc tto_find_random_floor_calls
    lda #30
    sta df_target_x
    lda #20
    sta df_target_y
...
test_tramp_teleport_other_execute:
    inc tto_spell_exec_calls
    lda pm_spell_idx
    sta tto_last_spell_idx
...
test_setup_teleport_other_room:
    jsr fill_map_rock
    lda #0
    sta vis_room_revealed
    sta tto_find_random_floor_calls
...
test_reset_teleport_other_state:
    ...
    sta tto_spell_exec_calls
    sta tto_huff_calls
    sta tto_last_huff_id
    sta tto_find_random_floor_calls
    sta vis_room_revealed
    sta zp_dirty_count
    lda #$ff
...
test_start:
    ...
    :PatchJump(test_spell_execute_selected, test_tramp_teleport_other_execute)
    ...
    :PatchJump(find_random_floor, test_find_random_floor_teleport_other)
    ...
    // 12 mana, and marks the spell worked.
    :PatchJump(calc_spell_failure, test_calc_spell_failure_success)
    jsr test_reset_teleport_other_state
    jsr test_setup_teleport_other_room
    lda #1
    sta test_mon_present
    jsr player_cast_spell
    bcc !t1_fail+
    lda tto_spell_exec_calls
    cmp #1
    bne !t1_fail+
    lda tto_last_spell_idx
    cmp #26
    bne !t1_fail+
    lda tto_find_random_floor_calls
    cmp #1
    bne !t1_fail+
    lda tto_huff_calls
    bne !t1_fail+

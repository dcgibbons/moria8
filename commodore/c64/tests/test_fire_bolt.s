// test_fire_bolt.s — Focused runtime tests for the Fire Bolt spell row
...
test_start:
    ...
    :PatchJump(test_spell_execute_selected, test_tramp_fire_bolt_execute)
    :PatchJump(pm_select_book, test_pm_select_book)
    :PatchJump(pm_prompt_visible_spell_choice, test_pm_prompt_visible_spell_choice)
    :PatchJump(pm_validate_selected_spell, test_pm_validate_selected_spell)

    // Test 1: successful cast reaches spell slot 22, kills the target through
    // the actual bolt owner, stays message-light, spends 9 mana, and marks
    // the spell worked in byte 2.
    :PatchJump(calc_spell_failure, test_calc_spell_failure_success)
    jsr test_reset_fire_bolt_state
...
!t1_fail:
    ...
    // Test 2: with no target in the traced direction, cast stays silent,
    // kills nothing, still spends mana, and still marks the spell worked.
!t2:
    :PatchJump(calc_spell_failure, test_calc_spell_failure_success)
    jsr test_reset_fire_bolt_state
    jsr tv_setup_dark_room
...
!t2_fail:
    ...
    // execute, and leaves Fire Bolt unworked.
!t3:

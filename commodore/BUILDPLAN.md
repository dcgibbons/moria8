...
| **LDR-1** | **BLOCKER** | C128 post-chargen town-entry `JAM` fixed by restoring the missing low-RAM runtime loader contract for `runtime_low.prg`, aligning the PRG header with `$1000`, and loading the callable VDC runtime code into Bank 0 before title/runtime rendering under `MMU_ALL_RAM`. | **2026-03-18** |

...
| **OPT-2** | **LOW** | `uv_player_in_room_x` now uses a cheaper expanded-bounds predicate for room membership, and focused regression coverage locks the perimeter-wall semantics down without changing LOS behavior. | **2026-03-20** |
| **REF-1** | **LOW** | The C128 trampoline surface is now consolidated into macroized families for compute calls, preserve-A/flags wrappers, UI display wrappers, banked status wrappers, and shared-epilogue special-room wrappers. The remaining bespoke trampolines stay explicit because their contracts are intentionally custom. | **2026-03-20** |

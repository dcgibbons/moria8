#importonce
// test_find_traps_prayer128.s — Focused C128 coverage for the Find Traps prayer row
...
test_start:
    ...
    lda #1
    sta test_progress
    jmp test_after_reveal
!t1_fail:
    jmp test_fail

    // Test 2: no hidden traps is a silent no-effect success that still
    // consumes mana, marks the prayer worked, and leaves non-trap tiles
    // unchanged under the current shared effect semantics.
test_after_reveal:
    :PatchJump(calc_spell_failure, test_calc_spell_failure_success)
    jsr test_reset_find_traps_state

#importonce
// test_sense_surroundings_prayer128.s — Focused C128 coverage for the Sense Surroundings prayer row
...
test_start:
    ...
    :PatchJump(tramp_spell_execute_selected, test_tramp_sense_surroundings_execute)

    // Test 1: successful prayer reaches slot 22, stays message-light, reveals
    // nearby floor and adjacent wall tiles, preserves hidden doors, spends 17
    // mana, and marks Sense Surroundings worked.
...
!t1_fail:
    ...
    // Test 2: inert-area success stays message-light, leaves inert tiles
    // unchanged, still consumes mana, and still counts as worked.
test_after_success:
    :PatchJump(calc_spell_failure, test_calc_spell_failure_success)
    jsr test_reset_sense_surroundings_prayer_state
    jsr fill_map_stairs_test
    lda #TILE_FLOOR | FLAG_OCCUPIED
    ldx #12
    ldy #22
    jsr test_write_tile

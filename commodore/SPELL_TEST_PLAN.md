...
| 24 | Slow Monster | M3 | Directional Control | C64+C128 | Slow valid target monster | target speed state mutates and target is no longer treated as fast/current-speed | Shared seam + spell-id mapping | Yes | Required: no target or unaffected target | success text per current behavior; unaffected/no-target contract required | Directional single-target + directional miss/no-target |
...
|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Detect Evil | P1 | Detect | C64+C128 | Cast with evil monster present | detect state enabled and evil-present result is reported under current Commodore semantics | Shared seam + spell-id mapping | Yes | Required: no evil monsters | explicit evil-present text; no-evil text; known Commodore deviation is current expected behavior | Detect result/no-result |
...
| 8 | Slow Poison | P1 | Utility Cleanse | C64+C128 | Cast while poisoned | poison severity decreases according to current rules | Shared seam + spell-id mapping | Yes | Required: not poisoned | success text if any must match current build; no-effect text or silence policy required | Timed buff/protection |
| 9 | Blind Creature | P2 | Directional Control | C64+C128 | Blind valid target monster | target blind/stun state mutates according to current implementation | Shared seam + spell-id mapping | Yes | Required: no target or unaffected target | success text per current behavior; no-target/unaffected contract required | Directional single-target + directional miss/no-target |
...
| 25 | Turn Undead | P3 | Dispel / Flagged Target | C64+C128 | Affect visible undead target(s) | eligible undead are turned or flee per current implementation | Shared seam + spell-id mapping | Yes | Required: no visible undead targets | success text if any must match current build; no-undead outcome required | Dispel / flagged-target |
...
| 27 | Dispel Undead | P4 | Dispel / Flagged Target | C64+C128 | Damage/remove visible undead target(s) | eligible undead take damage or die per current implementation | Shared seam + spell-id mapping | Yes | Required: no visible undead targets | success may be message-light; no-undead outcome required | Dispel / flagged-target |
...
| 29 | Dispel Evil | P4 | Dispel / Flagged Target | C64+C128 | Damage/remove visible evil target(s) | eligible evil targets take damage or die per current implementation | Shared seam + spell-id mapping | Yes | Required: no visible evil targets | success may be message-light; no-evil outcome text required | Dispel / flagged-target |
...
- Known documented deviations in `commodore/SPELLS.md` must be treated as current expected behavior until deliberately changed:
  - `Detect Evil` uses current Commodore detect semantics

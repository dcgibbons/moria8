...
| 12 | Cure Poison | M2 | Utility Cleanse | C64+C128 | Cast while poisoned | poison state clears | Dedicated row-level C64/C128 coverage | Yes | Required: not poisoned | poison-clear feedback on actual clear; already-clear remains silent; cast fail explicit | Timed buff/protection |
...
| 20 | Polymorph Other | M3 | Directional Control | C64+C128 | Polymorph valid ranged target monster along the chosen line | shared directional helper traces past the adjacent tile, and the target monster is replaced according to current invariants with map occupancy valid | Shared seam + spell-id mapping | Yes | Required: no target or resistant/unaffected target if current behavior supports it | success is message-light unless current build prints one; no-target/unaffected contract required | Directional line trace + deterministic RNG |
...
| 17 | Neutralize Poison | P3 | Utility Cleanse | C64+C128 | Cast while poisoned | poison state clears | Dedicated row-level C64/C128 coverage | Yes | Required: not poisoned | poison-clear feedback on actual clear; already-clear remains silent; cast fail explicit | Timed buff/protection |

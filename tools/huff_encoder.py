...
Usage:
  python3 tools/huff_encoder.py data/insult_strings.txt -o huffman_data.s
"""
...
def main():
    ...
    strings = []
    for line in lines:
        screencodes = [ascii_to_screencode(ch) for ch in line]
    ...
    for i, off in enumerate(offsets):
        out_lines.append(f'    .word ${off:04x}  // [{i}] "{lines[i][:30]}"')
    out_lines.append(f'')
    ...
    for i, data in enumerate(compressed):
        ...
        out_lines.append(f'    // [{i}] "{lines[i]}" ({orig_len}→{len(data)} bytes)')
        row = '    .byte '

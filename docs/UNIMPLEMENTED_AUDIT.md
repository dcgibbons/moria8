...
| --- | --- | --- | --- | --- |
| Monster infravision | C64, C128, and Plus/4 render warm monsters in darkness using race infravision and timed potion range. Terrain/items are not revealed. Warm-monster display is range-limited and blocked by the shared LOS trace, including closed doors and walls. | Implemented | `monster_update_visibility_one` and `CF_INFRA` in `core/monster.s`; `mm_los_clear_to_target` in `core/dungeon_los.s`; renderers consuming `MF_VISIBLE`; Umoria `monsterIsVisible`, `CD_INFRA`, `see_infra`; C128 live-map LOS coverage in `test_dungeon128.s` | Keep broad end-to-end play coverage as needed. |
| Infravision potion | Timer exists and now contributes +1 effective infravision range while active, matching Umoria's `timed_infra` increment model. | Implemented | `iq_effect_infravision` in `core/player_item_commands.s`; `player_get_infra_range` in `dungeon_los.s`; Umoria `player_quaff.cpp`, `game_run.cpp` | Add broader end-to-end coverage as needed. |
| Balrog victory | Balrog death sets a compact winner flag, blocks save-and-exit, routes Shift+Q through retirement and royal art, and marks winner scores. Focused coverage is still thin. | Implemented | `CREATURE_BALROG` in `platforms/commodore/c64/creature_data/creature_tiers.s`; `combat_note_kill` in `core/combat.s`; retirement path in `core/game_loop.s`; royal overlay in `core/royal.s` | Keep coverage follow-up in backlog. |
...
| Full monster catalog | Moria8 ships a selected tier roster rather than all 279 Umoria creatures; global monster IDs and complete recall persistence are not done. | Partial | `tools/parse_creatures.py`; `platforms/commodore/c64/creature_data`; `docs/MONSTERS.md` | Backlog item already exists. |
...
| Monster spells/breath breadth | Spell flags are compact and do not cover every upstream spell behavior. | Approximation | `tools/parse_creatures.py::map_spell_flags`; `core/monster_magic.s` | Covered by monster catalog/behavior expansion backlog. |
...
| Numeric repeat prefixes | Input code explicitly leaves numeric prefixes deferred; repeat count is always 1. | Missing | `platforms/commodore/c64/input.s`; `platforms/commodore/plus4/input.s` | Add backlog item. |
| Plus/4 TED sound | Sound backend is still silent despite TED hardware. | Missing | `platforms/commodore/plus4/sound.s`; `docs/BACKLOG.md` | Backlog item already exists. |

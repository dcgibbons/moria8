...
| --- | --- | --- | --- | --- |
| Monster infravision | C64, C128, and Plus/4 render warm monsters in darkness using race infravision and timed potion range. Terrain/items are not revealed. Exact wall-blocking LOS is still approximated by range. | Implemented | `monster_is_infra_visible_at` in `core/dungeon_los.s`; `CF_INFRA` in `core/monster.s`; C64/Plus4 renderers; C128 VDC renderer; Umoria `monsterIsVisible`, `CD_INFRA`, `see_infra` | Keep exact LOS blocking as backlog work. |

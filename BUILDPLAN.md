...
| BUG-30 | Combat messages corrupted (garbled PETSCII) for tier-loaded creature names | ✅ Fixed — `creature_get_name` returned stale name pointers from `cr_name_lo/hi` after overlays overwrote $E000. Fix: tier creatures now read name pointers directly from the $E000 region via `tier_name_lo_addr`/`tier_name_hi_addr` (computed in `tier_manager.s` after `load_tier_to_buffer`). Name bytes copied to `creature_name_buf` under KERNAL bank-out. |
...
| BUG-30 | **HIGH** | Combat messages corrupted (garbled PETSCII) for tier-loaded creature names | ✅ Fixed — Tier creatures now read name pointers directly from $E000 via `tier_name_lo_addr`/`tier_name_hi_addr`. |

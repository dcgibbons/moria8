...
- KickAssembler bootstrap belongs to the normal root Makefile path. Do not diagnose missing `tools/kickass/KickAss.jar` from platform-subdirectory make invocations or `make -B` behavior; rerun the normal root target first and separate tool bootstrap from real assembly errors.
...
- For upstream parity questions, verify the exact intended upstream before reusing a nearby local label. VMS trap death causes come from `take_hit(..., hit_from)` strings, which can differ from visible trap feature names; tombstone wording must follow the death cause, not the map/identify name.
